...
ARGV is the list of user args after the script; CWD the working directory."
  (let ((eng:*realm* realm)
    ...
    (let ((*runtime* rt))
      ...
      ;; tooling.hot-reload (#188): Clun.hot once hot-reload.lisp is loaded.
      (let* ((g (eng:realm-global realm))
             (clun (eng:js-get g "Clun"))
             (install-hot (find-symbol "%INSTALL-CLUN-HOT" :clun.runtime)))
        (when (and (eng:js-object-p clun) install-hot (fboundp install-hot))
          (funcall install-hot clun g)))
      ;; tooling.frontend-dev-server (#189): HTML entry loader + Clun.devServer.
      (%install-html-entry-loader)
      (let ((clun (eng:js-get (eng:realm-global realm) "Clun"))
            (g (eng:realm-global realm)))
        (when (eng:js-object-p clun)
          (%install-clun-dev-server clun g))))
