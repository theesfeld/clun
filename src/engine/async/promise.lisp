  ...
  (let ((pp (js-make-object (intrinsic :object-prototype))))
    ...
    (let ((ctor (make-constructor "Promise" 1
      ...
      (install-method ctor "withResolvers" 0
        (lambda (this args)
          (declare (ignore args))
          (multiple-value-bind (promise resolve reject)
              (new-promise-capability this)
            (let ((result (new-object)))
              (create-data-property-or-throw result "promise" promise)
              (create-data-property-or-throw result "resolve" resolve)
              (create-data-property-or-throw result "reject" reject)
              result))))
