...
(defun %bootstrap-global ()
  (let ((g (js-make-object (intrinsic :object-prototype))))
    ...
    (macrolet ((glob (name val) `(hidden-prop g ,name ,val)))
      ...
      ;; binary data (Phase 11) + shared memory (#338)
      ...
      (let ((sab (intrinsic :shared-array-buffer-constructor)))
        (when sab (glob "SharedArrayBuffer" sab)))
      (let ((atomics (intrinsic :atomics)))
        (when atomics (glob "Atomics" atomics)))
