...
| **m8 — Close the gate for deltablue + splay** | richards ✓. Levers (profile-guided): positional param binding (`bind-parameters` `nth`-walk is O(n²)); fast integer `ToString` (deltablue `"v"+i` → the rational number→string is `gcd`/`intexp` in the profile); wider ICs for IC-miss/polymorphic sites. | **G1** unchanged each step. **Gate G2 (≥5×)** evaluated across the trio (deltablue ≤588, splay ≤304 ms). |
| **(m9 — COMPILE tiering, conditional)** | Only if G2 is unmet after m8 (§5): background-thread `COMPILE` tier-up. If deltablue stays infeasible with behavior-preserving changes, a §2.4 scope note (§8.1 flagged ≥5× as "plausible, not guaranteed"). | **G2**: bench ratio reaches ≥5×. **G1** unchanged. |
