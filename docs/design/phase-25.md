...
| **m6 — Close on the ≥5× gate** | Profile-guided, one at a time: (a) a fast create-data-property path (skip the validate machinery for a new default data property on an extensible ordinary object — `make-prop-desc`/`validate-and-apply` ~15–20% on deltablue/splay); (b) positional param binding (the current `bind-parameters` walks the args list with `nth`, O(n²)); (c) shaped-`descs` simple-vector (kills the read-IC-hit hairy-`aref`). | **G1** unchanged each step. **Gate G2 (≥5×) evaluated here** across the trio (richards ≤720, deltablue ≤588, splay ≤304 ms). |
| **(m7 — COMPILE tiering, conditional)** | Only if G2 is unmet after m6 (§5): background-thread `COMPILE` tier-up. | **G2**: bench ratio reaches ≥5×. **G1** unchanged. |
