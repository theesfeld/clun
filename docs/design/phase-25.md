...
| **m4 — Write IC + `descs` simple-vector** | A per-site WRITE inline cache (shape→slot store, no `ordinary-set` scan) at the assignment set sites; move a shaped ptable's `descs` to a simple-vector to kill the residual ~15% hairy-`aref` on read-IC hits. | **G1** unchanged. **G2 slice**: incremental; expected to move deltablue/splay (write/alloc-bound). |
