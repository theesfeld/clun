...
| **m5 — Direct calls (+ string builder)** | Known-arity direct-call fast path (§4.1) — deltablue's dominant cost is `setup-frame` (~44% total: param binding + `arguments` allocation); then `+=` string builder (§4.2). A shaped-`descs` simple-vector (kills the residual read-IC-hit hairy-`aref`) is a smaller follow-up. | **G1** unchanged. **G2 slice**: incremental; string builder on a dedicated `s += t` micro-workload. **Gate G2 (≥5×) evaluated here** across the trio. |
