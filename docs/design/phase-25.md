...
| **m9 — small-integer string cache** ✓ | Shared `"0".."1023"` cache (`int->string`) for array index keys + integer `ToString`; unobservable sharing (JS strings immutable, all comparisons value-based). | **G1**: pass-list unchanged (22,643); review 0 HIGH/MEDIUM. **G2 slice**: richards **6.62×**, deltablue 3.81×, **splay 5.30× (MET)**. |
| **m10 — RESOLVED: COMPILE ceiling measured; option A off-ramp** ✓ | Operator first chose option C, so the source backend was widened through its m2 ceiling gate. Final default results: richards 6.68×, deltablue 3.85×, splay 5.36×; geomean 5.16×. Diagnostic eager compiled 72/72 DeltaBlue bodies but reached only 4.24× (694.6 ms), still above the 588.4 ms target. The preapproved off-ramp therefore cancels background-tier m3/m4 and accepts G2 on the majority/geomean basis. | **G2**: 2/3 + 5.16× geomean on frozen workloads; DeltaBlue explicitly documented. **G1**: all 22,643 frozen passes hold; off/eager classifications identical across 40,654 files; 0 crashes/fallback. |
