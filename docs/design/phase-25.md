...
| **m7 — Close the ≥5× gate** | Highest lever = property CREATION (`make-prop-desc`/`validate-and-apply` ~15–20% on deltablue/splay): a fast `create-data-property` path (new default data prop on an extensible ordinary `:object` → straight to `obj-set-desc`, skip validate-and-apply). Then positional param binding (the current `bind-parameters` `nth`-walk is O(n²)). | **G1** unchanged each step. **Gate G2 (≥5×) evaluated here** (richards ≤720, deltablue ≤588, splay ≤304 ms). |
| **(m8 — COMPILE tiering, conditional)** | Only if G2 is unmet after m7 (§5): background-thread `COMPILE` tier-up. | **G2**: bench ratio reaches ≥5×. **G1** unchanged. |
