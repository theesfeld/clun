...
| **m10 — OPEN §2.4 scope decision (deltablue)** | 2/3 met (richards 6.62×, splay 5.30×); geomean ≈5.1×; deltablue 3.81× (holdout — deep-proto dispatch + call machinery, the §8.1 tree-walker case). Options: (A) accept G2 on geomean/majority + document, proceed to 25b; (B) keep grinding behavior-preserving micro-opts (diminishing); (C) §5 background-thread `COMPILE` tier (large, ~post-v1). | **G2**: per operator's chosen basis. **G1** unchanged. |
