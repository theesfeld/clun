...
check(Clun.$`basename`, 1, "", "usage: basename string\n", "basename usage")
  ...
  .then(() => check(Clun.$`[[ 7 -eq 4+3 ]]`, 0, "", "", "conditional arithmetic addition"))
  .then(() => check(Clun.$`[[ 14 -eq 2+3*4 ]]`, 0, "", "", "conditional arithmetic precedence"))
  .then(() => check(Clun.$`[[ 20 -eq (2+3)*4 ]]`, 0, "", "", "conditional arithmetic grouping"))
  .then(() => check(Clun.$`IVAR=4+3; [[ $IVAR -eq 7 ]]`, 0, "", "", "expanded conditional arithmetic"))
  .then(() => check(Clun.$`A=7; [[ 7 -eq A ]]`, 0, "", "", "named conditional arithmetic"))
  .then(() => check(Clun.$`UNSET=; [[ 7 -gt $UNSET ]]`, 0, "", "", "empty conditional arithmetic"))
  .then(() => check(Clun.$`[[ 255 -eq 16#ff ]]`, 0, "", "", "based conditional arithmetic"))
  .then(() => check(Clun.$`EXPR='1|2'; [[ 3 -eq $EXPR ]]`, 0, "", "", "bitwise conditional arithmetic"))
  .then(() => check(Clun.$`[[ -1 -eq 18446744073709551615 ]]`, 0, "", "", "wrapped conditional arithmetic"))
  .then(() => check(Clun.$`[[ 7 -eq 4+ ]]`, 1, "", "clun: conditional expression: invalid arithmetic expression\n", "invalid conditional arithmetic"))
  .then(() => check(Clun.$`A=B; B=A; [[ 0 -eq A ]]`, 1, "", "clun: conditional expression: invalid arithmetic expression\n", "cyclic conditional arithmetic"))
  .then(() => check(Clun.$`[[ 1 -eq 1/0 ]]`, 1, "", "clun: conditional expression: invalid arithmetic expression\n", "zero division conditional arithmetic"))
