...
#+sbcl
;; clun purity patch (Phase 19): kept the pure sb-unix:unix-gethostname get-host-name;
;; dropped the Windows variant that reached "gethostname" through the alien API (§1.1).
(defun get-host-name ()
...
(progn
  ...
  ;; clun purity patch (Phase 19): the upstream body used an alien fd-set +
  ;; unix-fast-select. Replaced with sb-sys:wait-until-fd-usable — SBCL's pure
  ;; serve-event readiness primitive (§1.1). usocket is used only by pure-tls's
  ;; x509/crl.lisp (single-socket CRL fetch), so a per-socket wait suffices;
  ;; multi-socket timeout precision is a documented divergence.
  (defun wait-for-input-internal (sockets &key timeout)
    (with-mapped-conditions ()
      (dolist (waiter (wait-list-waiters sockets))
        (let ((fd (sb-bsd-sockets:socket-file-descriptor (socket waiter))))
