...
For RSA-PSS, /salt-length/ selects the exact salt length to encode or
verify. It defaults to the selected digest's output length for compatibility.

#+BEGIN_EXAMPLE
(sign-message rsa-private-key message :pss t :salt-length 20) => signature
(verify-signature rsa-public-key message signature :pss t :salt-length 20) => boolean
#+END_EXAMPLE
