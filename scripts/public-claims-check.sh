...
[ "$report_digest" = B74BA9D0C45F150C ] ||
  fail "current execution digest is not the frozen B74BA9D0C45F150C"
...
[ "$test262_passes" -eq 25793 ] && [ "$report_total" -eq 40654 ] &&
  [ "$report_pass" -eq 25793 ] && [ "$report_fail" -eq 2370 ] &&
  [ "$report_skip" -eq 12491 ] && [ "$report_crash" -eq 0 ] &&
  [ "$report_eligible" -eq 28163 ] && [ "$report_target" -eq 25347 ] &&
  [ "$report_lift" -eq 0 ] ||
  fail "execution artifacts no longer match the frozen 25,793/28,163 candidate result"
...
[ "$phase25b_rows" -eq 1767 ] && [ "$phase37_rows" -eq 603 ] ||
  fail "residual ownership no longer matches the frozen 1,767/603 split"
...
[ "$report_rate_exact" = 91.584703 ] ||
  fail "current exact pass rate is not the frozen 91.584703%"
