...
[ "$report_digest" = 8FCFC569AA653BF1 ] ||
  fail "current execution digest is not the frozen 8FCFC569AA653BF1"
...
[ "$test262_passes" -eq 25944 ] && [ "$report_total" -eq 40654 ] &&
  [ "$report_pass" -eq 25944 ] && [ "$report_fail" -eq 2219 ] &&
  [ "$report_skip" -eq 12491 ] && [ "$report_crash" -eq 0 ] &&
  [ "$report_eligible" -eq 28163 ] && [ "$report_target" -eq 25347 ] &&
  [ "$report_lift" -eq 0 ] ||
  fail "execution artifacts no longer match the frozen 25,944/28,163 candidate result"
...
[ "$phase25b_rows" -eq 1767 ] && [ "$phase37_rows" -eq 452 ] ||
  fail "residual ownership no longer matches the frozen 1,767/452 split"
...
[ "$report_rate_exact" = 92.120868 ] ||
  fail "current exact pass rate is not the frozen 92.120868%"
...
if [ "$release_state" = candidate ]; then
  ...
  require_text README.md "[Phase $active_phase]($active_issue_url) is complete."
  ...
  require_text site/index.html "Phase $active_phase is complete:"
