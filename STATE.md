...
(including #102 ls residual closes) disposition is **1,286 covered / 312 pending / 32
upstream-inactive**. `tooling.shell` remains **Partial**; no Yes claim. Master also holds Phase 51
WebSocket M0 at the same unpublished candidate `0.1.0-dev.21`.
**SemVer impact:** `patch`
...
source is `0.1.0-dev.21`. This unit is an **unpublished correction** of that candidate
(release-bearing shell behavior fix; no new prerelease slot). Phase 26 remains after Phase 82.
...
7 Partial / 14 No** with shell Partial and test-runner Partial on master; this unit does not change the
public matrix counts and claims no Yes.
