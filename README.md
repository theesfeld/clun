...
> Its release-bearing target is `0.2.0-dev.9` / `v0.2.0-dev.9` (SemVer impact: `patch`).
> The verified release boundary is `v0.2.0-dev.8`, with four native archives, checksums, Pages,
...
The published `v0.2.0-dev.8` boundary includes the built-in updater. After a later prerelease is
...
While the hosted boundary remains `v0.2.0-dev.8`, that command only reinstalls `v0.2.0-dev.8` and does not
add the updater.
...
The current source is the `0.2.0-dev.9` release candidate; the immutable tag and assets are not published yet.
The last published prerelease remains [`v0.2.0-dev.8`](https://github.com/theesfeld/clun/releases/tag/v0.2.0-dev.8).
...
./build/clun --version   # => clun 0.2.0-dev.9
```
